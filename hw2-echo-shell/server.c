...
static void process_requests(int listen_socket)
{
    ...
    while (1)
    {
        ...
        if (data_socket == -1)
        {
            perror("server: process_requests: failure to establish a connection");
            exit(EXIT_FAILURE);
        }
        ...
        if (ret == -1)
        {
            perror("server: process_requests: failure to receive request from client");
            close(data_socket);
            continue;
        }
        ...
        case CMD_PING_PONG:
            ...
            break;
        default:
            perror("server: process_requests: invalid command");
            close(data_socket);
            continue;
        }
        ...
        if (ret == -1)
        {
            perror("server: process_requests: failure to send response back to the client");
            close(data_socket);
            continue;
        }

        // Close the data socket for the next request
        close(data_socket);
    }
}
...
static void start_server()
{
    ...
    if (listen_socket == -1)
    {
        perror("server: start_server: failure to listen on socket");
        exit(EXIT_FAILURE);
    }
    ...
    if (ret == -1)
    {
        perror("server: start_server: failure to bind socket to socket name");
        exit(EXIT_FAILURE);
    }
    ...
    if (ret == -1)
    {
        perror("server: start_server: failure to listen");
        exit(EXIT_FAILURE);
    }
