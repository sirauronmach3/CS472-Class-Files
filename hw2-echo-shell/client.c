...
static int initParams(int argc, char *argv[], char **p)
{
    ...
    while ((option = getopt(argc, argv, ":p:c:")) != -1)
    {
        ...
        case 'c':
            ...
            break;
        case ':':
            perror("client: initParams: Option missing value");
            exit(-1);
        default:
        case '?':
            perror("client: initParams: Unknown option");
            exit(-1);
        }
    }
    *p = cmdBuffer;
...
static void init_header(cs472_proto_header_t *header, int req_cmd, char *reqData)
{
    ...
    header->cmd = req_cmd;
    header->dir = DIR_SEND;
    header->atm = TERM_FALL; // Assuming Fall term, you can change this as needed
    header->ay = 2022;       // Assuming the academic year is 2022, you can change this as needed
...
static void start_client(cs472_proto_header_t *header, uint8_t *packet)
{
    ...
    if (data_socket == -1)
    {
        perror("client: start_client: socket");
        exit(EXIT_FAILURE);
    }
    ...
    if (ret == -1)
    {
        perror("client: start_client: failure to connect");
        close(data_socket);
        exit(EXIT_FAILURE);
    }
    ...
    if (ret == -1)
    {
        perror("client: start_client: failure to send");
        close(data_socket);
        exit(EXIT_FAILURE);
    }
    ...
    if (ret == -1)
    {
        perror("client: start_client: failure to receive");
        close(data_socket);
        exit(EXIT_FAILURE);
    }
...
int main(int argc, char *argv[])
{
    ...
    case CMD_PING_PONG:
        ...
        break;
    default:
        perror("client: main: usage requires zero or one parameter");
        exit(EXIT_FAILURE);
    }
